...
const showMinMax = function (expenses) {
  ...
  const minList = document.createElement("li");
  const maxList = document.createElement("li");
  minList.innerText = `Min: $${min}`;
  maxList.innerText = `Max: $${max}`;

  results.append(minList, maxList);
